        ...
        post_form service_save_page_post
          (fun chain ->
            [(p [string_input ~input_type:`Submit ~value:"Save" (); br ();
                 ...
                 ~value:wikitext ()]);
	     div [iframe ~a:[a_id "wysiframe"; 
(*			     a_src "#" ; *)
			     a_onload {{ 
			       let el = Js.Opt.get 
				 (Dom_html.document##getElementById (Js.string "wysiframe")) 
				 (fun () -> assert false)
			       in
			       let frame: Dom_html.iFrameElement Js.t = Js.Unsafe.coerce el in
			       frame##style##border <- Js.string "2px green solid";
			       frame##src <- Js.string "#";
			       frame##contentWindow##alert (Js.string "hallo");
			       Lwt.return ()
			     }}] []]
	     ])
          page
